...
export function FinalResultsPage({ finalResults, onRestart }: Props) {
  const chartData = finalResults.formatAverages
    ...
    .map(r => ({ name: r.formatCode, fullName: r.formatName, percent: r.percent, color: r.color }));
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-6">
      <div className="max-w-4xl mx-auto">
        ...
        {/* Средняя эффективность по форматам */}
        <div className="bg-gray-800 rounded-2xl p-6 mb-8 border border-gray-700">
          <h2 className="text-lg font-semibold mb-4">Ваш средний % попадания в форматы</h2>
        ...
        {/* Таблица */}
        <div className="bg-gray-800 rounded-2xl p-6 mb-8 border border-gray-700">
          <h2 className="text-lg font-semibold mb-4">Детали по форматам</h2>
            ...
            {finalResults.formatAverages.sort((a, b) => b.percent - a.percent).map(r => (
              <div key={r.formatCode} className="flex items-center gap-4">
                ...
                <div className="flex-1 bg-gray-700 rounded-full h-4">
                  <div className="h-4 rounded-full" style={{ width: `${r.percent}%`, backgroundColor: r.color }} />
                </div>
        ...
        <div className="text-center">
          <button onClick={onRestart} className="bg-orange-500 hover:bg-orange-600 px-10 py-4 rounded-xl font-semibold text-lg transition">
            Попробовать снова
          </button>
        </div>
      </div>
